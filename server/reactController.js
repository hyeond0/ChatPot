...
export const postReact = async (req, res) => {
  ...
  messages.push({ role: "assistant", content: recipeString });

  let recipeObject = parsingRecipe(recipeString);
  recipeObject.messages = messages;

  res.json(recipeObject);
};

export const reRecipe = async (req, res) => {
  let { messages } = req.body;
  messages.shift();

  messages.push({
    role: "user",
    content: "다른 레시피를 같은 옵션으로 하나만 추천해 줘.",
  });
...
function parsingRecipe(recipeString) {
